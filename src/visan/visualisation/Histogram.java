...
public class Histogram extends DataComponent {
    ...
    public Histogram(ClassesCollection cc, int[] classInd, int[] featureInd) {
        super("histogram", cc);
    ...
    public void setBins(int n) {
        numberOfBins = n;
        points = new int[classes.length][numberOfBins];
        refresh();
    }
        ...
        if (!histogramShape) {
            ...
            for (int j = 0; j < numberOfBins; j++) {
                ...
                for (int i = 0; i < classes.length; i++) {
                    ...
                    g2d.drawString(col.getClassesNames()[classes[ci]], 5 + (ci / 3) * 60, height - 30 + margin + 20 * (ci % 2) + step);
                    if (points[ci][j] > 0) {
